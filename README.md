...
SERVICES/API.JS: instância do axios com a baseURL do backend. no navegador, a URL da api é montada a partir do próprio endereço da página (quem abre em localhost:5174 fala com localhost:3001), então o site não depende do IP do PC e continua funcionando quando você troca de rede wi-fi. só no app do celular (detectado pelo capacitor) isso não funciona, porque lá a página é servida pelo próprio app e não dá para deduzir onde está o PC: o app usa o endereço que a pessoa salvou nele pelo botão "trocar IP" (fica guardado no aparelho) ou, se nunca salvou, o IP gravado no build. as requisições têm timeout de 10 segundos: se a api não responder, a tela mostra um aviso em vez de ficar em "carregando" para sempre. o arquivo também exporta mensagemDeErro (transforma o erro do axios em uma mensagem legível) e avisarEstoqueAtualizado (dispara um evento para a barra lateral recarregar os números depois de uma entrada, saída ou contagem).
...
COMPONENTS/ERROCARREGAMENTO.JSX: o aviso com borda vermelha e botão "tentar de novo" que aparece em qualquer página quando a api não responde. no app do celular ele mostra também o endereço que o app tentou usar e um botão "trocar IP", para corrigir o IP do PC na hora sem gerar outro apk.
...
como o app roda no celular, ele não enxerga o localhost do PC — precisa do IP do PC na rede wi-fi. o build-android.ps1 descobre esse IP sozinho na hora de gerar o apk (pela placa de rede que está com a internet) e grava no app. se quiser forçar outro IP: .uild-android.ps1 -Ip 192.168.0.10. o Frontend/.env (VITE_API_URL) só é usado se você gerar o app sem o script. se o IP do PC mudar depois do app instalado, não precisa gerar outro apk: quando o app não conseguir conectar, aparece o botão "trocar IP" para digitar o IP novo.

O APP É SERVIDO EM HTTP DE PROPÓSITO: no capacitor.config.json está androidScheme: "http". por padrão o capacitor abre o app em https://localhost, e o android bloqueia uma página https de chamar uma api http (erro "mixed content" no logcat) — ou seja, com o padrão o app nunca conseguia falar com o backend, que roda em http na rede local. não volte para https sem colocar o backend em https também.
...
requisitos para o app funcionar: celular e PC na mesma rede wi-fi, e o backend rodando no PC. atenção com redes de visitantes/faculdade/empresa: muitas isolam os aparelhos entre si, e aí o celular não enxerga o PC mesmo com tudo certo. nesse caso use uma rede de casa ou o roteador do celular. os detalhes de como o build foi configurado (JDK, android SDK, e por que existe o script em vez de rodar o gradlew direto) estão comentados no topo do próprio build-android.ps1.
